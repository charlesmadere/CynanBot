...
class LocationsRepository():
    ...
    def __init__(
        ...
        if locationsFile is None or len(locationsFile) == 0 or locationsFile.isspace():
            raise ValueError(
                f'locationsFile argument is malformed: \"{locationsFile}\"')
    ...
    def getLocation(self, id_: str):
        ...
        if not os.path.exists(self.__locationsFile):
            raise FileNotFoundError(
                f'Locations file not found: \"{self.__locationsFile}\"')
        ...
        if jsonContents is None:
            raise IOError(
                f'Error reading from locations file: \"{self.__locationsFile}\"')
        ...
        raise RuntimeError(
            f'Unable to find location with ID \"{id_}\" in locations file: \"{self.__locationsFile}\"')
