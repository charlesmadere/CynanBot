...
class GoogleApiService(GoogleApiServiceInterface):
    ...
    def __init__(
        ...
        elif not utils.isValidStr(projectId):
            raise TypeError(f'projectId argument is malformed: \"{projectId}\"')
        elif not isinstance(timber, TimberInterface):
            raise TypeError(f'timber argument is malformed: \"{timber}\"')

        self.__googleJsonMapper: GoogleJsonMapperInterface = googleJsonMapper
        self.__networkClientProvider: NetworkClientProvider = networkClientProvider
        self.__projectId: str = projectId
    ...
    async def translate(self, request: GoogleTranslationRequest) -> GoogleTranslateTextResponse:
        ...
        try:
            response = await clientSession.post(
                url = f'https://translate.googleapis.com/v3beta1/{self.__projectId}:translateText'
            )
