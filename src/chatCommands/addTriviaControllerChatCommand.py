    ...
    async def handleChatCommand(self, ctx: TwitchContext):
        ...
        if len(splits) < 2:
            ...
            await self.__twitchUtils.safeSend(
                messageable = ctx,
                message = f'⚠ Unable to add trivia controller as no username argument was given. Example: !addtriviacontroller {twitchHandle}',
        ...
        if not utils.isValidStr(userName):
            ...
            await self.__twitchUtils.safeSend(
                messageable = ctx,
                message = f'⚠ Unable to add trivia controller as username argument is malformed. Example: !addtriviacontroller {twitchHandle}',
                replyMessageId = await ctx.getMessageId()
            )
            return
        ...
        match result:
            ...
            case _:
                ...
                self.__timber.log('AddTriviaControllerChatCommand', f'Encountered unknown AddTriviaGameControllerResult value ({result}) when trying to add \"{userName}\" as a trivia game controller for {ctx.getAuthorName()}:{ctx.getAuthorId()} in {user.handle}')
