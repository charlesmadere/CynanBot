...
class RemoveRecurringWordOfTheDayActionCommand(AbsChatCommand):
    ...
    async def handleChatCommand(self, ctx: TwitchContext):
        ...
        if userId != ctx.getAuthorId() and administrator != ctx.getAuthorId():
            self.__timber.log('RemoveRecurringWordOfTheDayActionCommand', f'{ctx.getAuthorName()}:{ctx.getAuthorId()} in {user.handle} tried using this command!')
            return
        ...
        if recurringAction is None:
            self.__twitchChatMessenger.send(
                text = f'⚠ Your channel has no recurring word of the day action',
                twitchChannelId = await ctx.getTwitchChannelId(),
            ...
            self.__twitchChatMessenger.send(
                text = f'⚠ Your channel\'s recurring word of the day action is already disabled',
                twitchChannelId = await ctx.getTwitchChannelId(),
                replyMessageId = await ctx.getMessageId(),
            )
            return
        ...
        self.__twitchChatMessenger.send(
            text = f'ⓘ Recurring word of the day action has been disabled',
            twitchChannelId = await ctx.getTwitchChannelId(),
            replyMessageId = await ctx.getMessageId(),
        )

        self.__timber.log('RemoveRecurringWordOfTheDayActionCommand', f'Handled command for {ctx.getAuthorName()}:{ctx.getAuthorId()} in {user.handle}')
