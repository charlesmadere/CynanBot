...
class TimeoutCheerActionHelper(TimeoutCheerActionHelperInterface):
    ...
    async def __determineRandomTimeoutTarget(
        ...
        if randomNumber <= additionalReverseProbability:
            self.__timber.log('TimeoutCheerActionHelper', f'Attempted to timeout random target from {cheerUserName}:{cheerUserId} in {user.handle}, but they got the additional reverse probability RNG ({additionalReverseProbability=}) ({randomNumber=}) ({timeoutAction=})')
        ...
        if len(eligibleChatters) == 0:
            self.__timber.log('TimeoutCheerActionHelper', f'Attempted to timeout random target from {cheerUserName}:{cheerUserId} in {user.handle}, but no active chatter was found ({additionalReverseProbability=}) ({randomNumber=}) ({timeoutAction=}) ({chatters=})')
            return None
